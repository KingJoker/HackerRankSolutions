...
Each element in the collection is accessed using an index, and the elements are
...
Each integer storage cell is assigned a unique index ranging from 0 to one less than the size of the array,
and each cell initially contains a 0.
...
1. Modify the code in the loop so that it saves each sequential value to its corresponding location in the array.
2. For example, the first value must be stored in a[0], the second value must be stored in a[1], and so on.
Good luck!
*Input Format
