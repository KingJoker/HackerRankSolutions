...
*Explanation
We have two queries:
1. We use a=5, b=3, and n=5 to produce some series s[0],s[1],...,s[n-1]:
s[0]=0+1*2=2
s[1]=0+1*2+2*2=6
s[2]=0+1*2+2*2+4*2=14
... and so on.
Once we hit n=10, we print the first ten terms as a single line of space-separated integers.
We use a=5, b=3, and n=5 to produce some series s[0],s[1],...,s[n-1]:
s[0]=5+1*3=8
s[1]=5+1*3+2*3=14
...
class JavaLoops2{
    public static void main(String []argh){
        ...
        for(int i=0;i<t;i++){
            ...
            int previous = a;
            for(int j = 0; j < n; j++){
                int current = previous + (int)Math.pow(2,j) * b;
