...
2
Explanation 0
We must determine the total number of notifications the client receives over a period of 9 days.
...
The client spends 6 dollars, which triggers a notification because 6>=2*median: notifications=0+1=1.
On the seventh day, the bank has d=5 days of prior transaction data, {3,4,2,3,6}, and median=3 dollars.
The client spends 8 dollars, which triggers a notification because 4<2*median: notifications=2.
On the eighth day, the bank has d=5 days of prior transaction data, {4,2,3,6,8}, and median=4 dollars.
The client spends 4 dollars, which does not trigger a notification because 4<2*median: notifications=2.
On the ninth day, the bank has d=5 days of prior transaction data, {2,3,6,8,4}, and a transaction median of 4 dollars.
The client spends 5 dollars, which does not trigger a notification because 5<2*median: notifications=2.
*Sample Input 1
5 4
1 2 3 4 4
*Sample Output 1
...
public class FraudulentActivityNotifications {
    ...
    static int activityNotifications(int[] expenditure, int d) {
        ...
        for(int i = 0; i < expenditure.length; i++){
            if(trailing.size() >= d){
                if(expenditure[i] >= (2 * median(trailing))){
                    notices++;
                }
                int removeIndex = Collections.binarySearch(trailing,expenditure[i-d]);
                trailing.remove(removeIndex);
            }
            int insertIndex = Collections.binarySearch(trailing,expenditure[i]);
            if(insertIndex < 0){
                insertIndex = (insertIndex + 1) * -1;
            }
            trailing.add(insertIndex,expenditure[i]);
        }
        return notices;
    }
    ...
    static double median(ArrayList<Integer> list){
        if(list.size() % 2 == 0){
            int leftIndex = list.size() / 2 - 1;
            int rightIndex = leftIndex + 1;
            return (list.get(leftIndex) + list.get(rightIndex))/2.0;
        }
