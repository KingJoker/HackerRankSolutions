...
public class RepeatedString {
    ...
    static long repeatedString(String s, long n) {
        long numAsInitial = s.chars().filter((c) -> c == 'a').count();
        long numAsPartial = s.substring(0, (int)(n % s.length())).chars().filter((c) -> c == 'a').count();
        long numAsWhole = (n / s.length()) * numAsInitial;
        return numAsWhole + numAsPartial;
    }
