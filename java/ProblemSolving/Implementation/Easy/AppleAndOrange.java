...
Given the value of d for m apples and n oranges, determine how many apples and oranges will fall
on Sam's house (i.e., in the inclusive range [s,t])?
...
and oranges=[3,-2,-4] units distance. Adding each apple distance to the position of the tree, they land at [4+2,4+3,4+ -4].
Oranges land at [12+3,12+ -2,12+ -4. One apple and two oranges land in the inclusive range 7<->10 so we print
1
2
...
public class AppleAndOrange {
    ...
    static void countApplesAndOranges(int s, int t, int a, int b, int[] apples, int[] oranges) {
        long appleCount = Arrays.stream(apples).filter(apple -> {
            int position = a + apple;
            return position >= s && position <= t;
        }).count();
        long orangeCount = Arrays.stream(oranges).filter(orange -> {
